...
def cross_valid(forecast_model, cv_dict, model, metrics):
    data, features, target = forecast_model.data, forecast_model.features + forecast_model.date_time, forecast_model.targets
    cv = cv_dict['cv']
    cv_split = list(cv.split(len(data), cv_dict['step'], cv_dict['gap']))
    
    results = {'train_residuals' : np.zeros((cv.train_size,1)),
               'test_residuals' : np.zeros((cv.test_size, 1)),
               'mean_Y' : np.zeros((cv.train_size,1)),
    ...
    for train, test in cv_split:
        ...
        results['train_residuals'] += Y_train - Y_train_pred
        results['test_residuals'] += Y_test - Y_pred
    ...
    results['mean_Y_train_pred'] /= cv.n_splits
...
def run_cv(data, targets, horizons, CrossValid_params, ForecastModel_params, model, metrics):
    ...
    for tar in targets:
        quality[tar] = {'train_residuals' : [], 'test_residuals' : [], 
                        'train_loss' : [], 'test_loss' : [], 'mean_Y' : [], 'mean_Y_pred' : [],
                        'mean_Y_train_pred' : [], 'mean_Y_test' : [], 'forecast_model' : [], 'importances' : []}
    for hor in range(1, horizons + 1):
        for tar in targets:
            forecast_model = ForecastModel(date_time, data, features, [tar], prior_lag = list(range(hor,prior_lag + 1)),
                                           post_lag = post_lag)
            ...
            quality[tar]['train_residuals'].append(results['train_residuals'])
            quality[tar]['test_residuals'].append(results['test_residuals'])
...
def plot_cv_results(quality, horizons, plot_loss = False):
    for key in quality.keys():
        ...
        plt.plot(test_index, np.reshape(quality[key]['mean_Y_pred'],(horizons,1)), alpha = 0.65, label = 'mean_Y_pred')
        plt.legend()
        plt.show()
        if plot_loss:
...
def get_residuals(quality, horizons):
    resid_dict = {}
    for key in quality.keys():
        cv = quality[key]['cv_dict']['cv']
        resid_dict[key] = {}
        train_index, test_index = list(range(cv.train_size)), list(range(cv.train_size,cv.train_size + horizons))
        train_resid = pandas_Series(data = np.reshape(reduce(lambda a, b: a + b, quality[key]['train_residuals']) / horizons, (len(train_index),)), index = train_index , name = 'train_resid')
        test_resid = pandas_Series(data =  np.reshape(quality[key]['test_residuals'], (horizons,)), index = test_index, name = 'test_resid')
        resid_dict[key]['train_resid'], resid_dict[key]['test_resid'] = train_resid, test_resid
    return resid_dict
